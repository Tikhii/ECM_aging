...
**Revision history**:
- 2026-05-02 (子阶段 6 of `release/v0.7.0-fit4`, ADR-0016): R_SEI 退出 FIT-4a fittable params, 5-param → 4-param; 来源改为 `cell.aging.resistance_aging.R_SEI` (`param_specs/*.params.json` literature_default 加载, 或 EXP-E IR 脉冲实测覆盖).
- 2026-05-07 (子阶段 B.1 of `release/v0.8.0-lfp-integration`, ADR-0021+0022): LFP/graphite 第二 cell_type 接入. 新增 `lfp_large_format_power.material.json` + `lfp_large_format_power__mmeka2025.params.json` + `LFPAlawa.dat` (Padhi/Yamada 派生); `cell_types` 派发顶层字段引入. SPEC frozen 标记不动.
...
- **FIT-4a (calendar)**: 拟合 calendar rate constants from EXP-E (`k_SEI_cal`, `k_LAM_PE_cal`, `gamma_PE`, `E_a_SEI`). Rate law: paper Eq. 36 (SEI calendar 项), Eq. 40 (LAM_PE calendar 项), Eq. 45 (NE 电阻退化因子). `R_SEI` 不在 fittable params 内 (ADR-0016): 来自 `cell.aging.resistance_aging.R_SEI` (cell_factory 加载 `param_specs/*.params.json` literature_default, 或 EXP-E IR 脉冲实测覆盖).
...
下表 8 条均不在 FIT-4 中再做选择性修正; 实施按既定路径处理. 详见 `docs/CRITICAL_REVIEW.md` 对应小节 (paper errata 与 simplifications 已在 v0.5.3 之前事实层登记完毕, 本 SPEC 仅作引用并标注 FIT-4 实施后果).
...
| 7 | SPEC 误归类 | 本 SPEC v0.7.0 frozen 时把 `R_SEI` 列入 FIT-4a fittable params, 但 R_SEI 在 calendar (I=0 storage) 模式下完全不可识别 (J 列恒 0, J^TJ 奇异); ADR-0016 修订: R_SEI 来自 EXP-E IR 脉冲直接测量 (paper p.12 §Calendar degradation), 不从 LLI 序列反推 | FIT-4a free params 5 → 4; R_SEI 来源 = `cell.aging.resistance_aging.R_SEI` (cell_factory 加载 `param_specs/*.params.json` literature_default 0.66, 或 EXP-E IR 实测覆盖) | `docs/decisions/0016-r-sei-exit-calendar-fittable-params.md`; `aging_kinetics.py::ResistanceAgingParameters.R_SEI` |
| 8 | LFP V_LP_eq 简化 | LFP cell_type 锂沉积平衡势 V_LP_eq=0 V (沿用 Mmeka 2025 paper Eq. 39 简化, NCR 同). k_LP bounds B.3 LFP 实测 calibrate. R_SEI literature_default 起步 (用户 EXP-E v0.8 不强求). | ADR-0016 类似处理: cell_type 间分支语义统一, 数值 calibrate 留 fit step. | `docs/decisions/0021-lfp-llionly-ic-analysis.md`; `param_specs/lfp_large_format_power__mmeka2025.params.json` |
...
**Cell type routing** (v0.8 B.1, ADR-0021):

| cell_type | FIT-4a | FIT-4b | FIT-4c | IC analysis 输入 |
|---|---|---|---|---|
| `panasonic_ncr18650b` (NCA/Graphite) | 完整 (4 free params: `k_SEI_cal, k_LAM_PE_cal, gamma_PE, E_a_SEI`) | 完整 (3 free params: `k_SEI_cyc, k_LAM_PE_cyc, k_LAM_NE_cyc`) | 完整 (1D: `k_LP`) | 三 DM (LLI, LAM_PE, LAM_NE) |
| `lfp_large_format_power` (LFP/Graphite) | LAM_PE 项跳过 (输入 `LAM_PE_Ah=NaN`); 仅 `k_SEI_cal, E_a_SEI` 拟合 | LAM_PE / LAM_NE 项跳过 (`LAM_*_Ah=NaN`); 仅 `k_SEI_cyc` 拟合 | 完整 (`k_LP`, LLI 输入有效) | LLI-only (LAM_*_Ah → NaN, ADR-0021) |

LFP 分支 IC analysis 输出 `ICAnalysisResult.LAM_PE_Ah / LAM_NE_Ah = NaN` 时, FIT-4a/4b 自动 skip 对应 rate constant 拟合 (参 ADR-0021 + `docs/SPEC_ic_analysis.md` §LFP LLI-only branch). FIT-4c 不受影响, `k_LP` 拟合输入 LLI 仍 valid.
...
FIT-4a (calendar)
  ...
  ├ free: {k_SEI_cal, k_LAM_PE_cal, gamma_PE, E_a_SEI}  # R_SEI 退出 (ADR-0016): 来源 cell.aging.resistance_aging.R_SEI
...
R3 (resistance LUT 仅 fresh-cell): 三 stage cost_fn 内部 forward 仿真时, `R_s_LUT` / `R_NE_LUT` / `R_PE_LUT` 不重测; 老化期间电阻通过 `f_R_PE`, `f_R_NE`, `f_Rs` 因子自动应用. `R_SEI` 来自 EXP-E IR 脉冲直接测量 (paper p.12), 在 cell_factory 加载阶段经 `param_specs/*.params.json` 提供 literature_default; FIT-4a 不再拟合 R_SEI (ADR-0016).
