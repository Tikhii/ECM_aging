...
> PARAMETERS.json::critical_review_findings, CLAUDE.md 任务路由表, CHANGELOG.md
> 版本条目) 仍在 R5 范围内
...
`docs/legacy/MIGRATION_NOTES.md` §20.6.
...
- §4 候选改进路径升级到 v0.6+ / v0.7+ 实施 → 在 `docs/decisions/` 以 ADR
  形式记录决策 rationale, 并在 `CHANGELOG.md` 追加 entry 引用该 ADR;
  本文对应小节标 "已升级" 状态
...
| `CHANGELOG.md` + `docs/decisions/` | v0.5.3 patch 决策记录 | CHANGELOG 追加 entry, 必要时新增 ADR |
