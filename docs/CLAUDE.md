...
R5 验收阶段管的是文档层超范围审查, 本节补充代码层。修改既有代码时:

- **匹配既有风格**: 编辑既有代码时, 匹配既有代码风格 (命名 / quote / import 顺序 / docstring 格式 / 等)。即使你认为另一种风格更好, 在本次范围外不重排。如发现工程风格不一致, 以警告形式呈现在完成报告中, 不据此自行统一化。
- **未使用代码**: 你的改动产生的孤立 import / 变量 / 函数应清理。**预先存在**的疑似死代码 (本次改动之前就存在的) mention 但不删, 留 audit trail 由用户决策。
- **PR 自检**: 每行 diff 应能追溯到用户原请求。无关改动 (refactor 顺手 / docstring 美化 / 邻近 bug 顺手修) 剥离到独立 commit, 或推迟到本次任务之外。
...
- `pytest tests/ -v` 全绿
- `git diff --stat` 呈给用户
- **大改动 commit 前 read 关键文件全文一次, 不只看 diff**: 工具 (`edit_file` / `str_replace` 等) 的 diff 预览偶有失真 (例如 `edit_file` 空 `edits=[]` 时 diff 显示像写入但实际未写入)。 commit 前对关键改动文件 read 全文实证最终状态, 不基于工具 diff 预览推断 "已写入"。
- **多次发布的工程数字与之前报告对齐核对**: 工程数字 (改动行数 / commit 大小 / 测试 PASS 数) 在新阶段时主动核对前阶段报告。例如子阶段 N commit 前 `git diff --cached --stat` 输出对照子阶段 N 报告内的预期数字, 差异需具体溯源 (微调 / 增删 / 工具偏差), 不接受 "差不多"。
