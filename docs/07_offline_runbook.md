...
> 而非 "缺失率 > 32.4%")。这与 `docs/08_consultation_protocol.md` §1/§2 的
> 外带信息白名单保持一致。
...
在线咨询" 的指引, 用 `docs/08_consultation_protocol.md` §3 的观测笔记模板把
...
- `environment.yml`
- `TODO: environment-frozen.yml 锁定机制尚未落盘 (2026-04 讨论决定的产物)`
...
**何时升级到在线咨询**
见 `docs/08_consultation_protocol.md §5.例 3 — 环境漂移`。
...
**何时升级到在线咨询**
见 `docs/08_consultation_protocol.md §5.例 1 — 数据契约违规`。
...
→ 数据契约违规的统一外带模板见 `docs/08_consultation_protocol.md §3`。
...
**何时升级到在线咨询**
见 `docs/08_consultation_protocol.md §5.例 2 — 拟合前置条件不足`。
...
**何时升级到在线咨询**
见 `docs/08_consultation_protocol.md §5.例 4 — R2 顺序违规`。
...
**何时升级到在线咨询**
见 `docs/08_consultation_protocol.md §5.例 5 — 求解器数值异常`。
...
**何时升级到在线咨询**
见 `docs/08_consultation_protocol.md §5.例 6 — 识别性警告`。
